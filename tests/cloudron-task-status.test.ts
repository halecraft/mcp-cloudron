...
describe("cloudron_task_status tool", () => {
  ...
  it("should get task status for successful task with result data", async () => {
    ...
    const result = taskStatus.result as { backupId: string; size: number }
    expect(result.backupId).toBe("backup-20241223-140000")
    expect(result.size).toBe(1024000000)
  })
  ...
  it("should handle 404 for invalid task ID", async () => {
    ...
    await expect(client.getTaskStatus("invalid-task")).rejects.toThrow(
      "Task not found",
    )
  })
  ...
  it("should validate all task states", async () => {
    const states: Array<"pending" | "running" | "success" | "error"> = [
      "pending",
      "running",
      "success",
      "error",
