...
describe("cloudron_get_logs", () => {
  ...
  describe("Error handling", () => {
    it("should return 404 for invalid app ID", async () => {
      ...
      await expect(client.getLogs("invalid-app", "app")).rejects.toThrow(
        "App not found",
      )
    })
    ...
    it("should return 404 for invalid service ID", async () => {
      ...
      await expect(
        client.getLogs("invalid-service", "service"),
      ).rejects.toThrow("Service not found")
    })

    it("should reject invalid type parameter", async () => {
      await expect(client.getLogs("app-123", "invalid" as any)).rejects.toThrow(
        "Invalid type: invalid. Valid options: app, service",
      )
    })

    it("should require resourceId parameter", async () => {
      await expect(client.getLogs("", "app")).rejects.toThrow(
    ...
    it("should handle authentication errors", async () => {
      ...
      await expect(client.getLogs("app-123", "app")).rejects.toThrow(
        "Invalid token",
      )
    })
