...
server.setRequestHandler(CallToolRequestSchema, async request => {
  ...
  Used: ${storageInfo.used_mb} MB`
      ...
      case "cloudron_validate_operation": {
        const { operation, resourceId } = args as {
          operation: ValidatableOperation
          resourceId: string
        }
        const validationResult = await cloudron.validateOperation(
          operation,
          resourceId,
        )
...
Use cloudron_task_status with taskId '${result.taskId}' to track completion.`,
      ...
      case "cloudron_configure_app": {
        ...
        const configChanges = Object.keys(config)
          .map(key => {
            if (key === "env") {
        ...
        const formatted = backups
          .map((backup, i) => {
            ...
            const size = backup.size
              ...
              : "N/A"
            const appCount =
              backup.appCount !== undefined ? backup.appCount : "N/A"
...
${errorsText}${warningsText}`,
      ...
      case "cloudron_create_user": {
        const { email, password, role } = args as {
          email: string
          password: string
  ...
  TLS: ${d.tlsConfig.provider} (wildcard: ${d.tlsConfig.wildcard})`,
      ...
      case "cloudron_get_logs": {
        const { resourceId, type, lines } = args as {
          resourceId: string
          type: LogType
          lines?: number
        }
